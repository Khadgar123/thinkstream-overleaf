\begin{figure*}[t]
  ...
  \caption{Motivation for Watch-Think-Interact. Streaming video reasoning must answer asynchronous queries with a bounded active window, where future frames are unavailable and earlier evidence may fall out of view. WTI supports waiting, active-window answering, and historical recall through compact time-indexed memory and time-bounded recall.}
...
Complementary to cache and state reuse, visual-token compression, hierarchical compression, and event-level memory further reduce dense video context~\citep{longvu2025,videochatflash2026,streamforest2025}.
...
To train Watch-Think-Interact, we construct WTI-82K, a dataset of chunk-level streaming interaction trajectories built under strict streaming causality.
...
\begin{itemize}[leftmargin=*, itemsep=1pt, topsep=1pt, parsep=1pt, partopsep=1pt]
  ...
  \item We construct WTI-82K, a causally timed dataset for streaming interaction training.
