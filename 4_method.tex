...
\begin{figure*}[t]
  ...
  \caption{Overview of Watch-Think-Interact. At each stream step, the agent watches the current chunk, updates a thinking state, selects silence, response, or recall, and writes compact time-indexed memory at context boundaries; Stream-GDPO optimizes the resulting streaming trajectories.}
...
\end{equation}
The action \(\mathrm{silence}\) delays output, while \(\mathrm{response}\) commits an answer to the current question.

When past visual evidence is needed, the agent may invoke a recall tool to inspect a previously observed interval.
The call \(\mathrm{recall}(\tau_s,\tau_e)\) is valid only when \(\tau_s\le\tau_e<t\).
The environment returns evidence from this interval, which is added to the current decision context before the agent either answers or remains silent at that step.
...
\begin{figure*}[t]
  ...
  \caption{ChronoInteract-82K construction pipeline. Videos are standardized, chunk-level evidence chains are aligned, evidence-grounded QAs are generated, query and answer times are placed on the streaming timeline, and QA quality and causal consistency are verified.}
  \label{fig:data_pipeline}
\end{figure*}

\input{tables_ovo_offline_main}
...
Building on this setting, we construct ChronoInteract-82K, a time-aware dataset for long-horizon multi-turn streaming video reasoning.
